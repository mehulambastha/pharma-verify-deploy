...
const Page = () => {
  ...
  const handleApprove = (drugName) => {
    let drugToApprove = allProducts.find(drug => drug["Medicine Name"] == drugName)

    drugToApprove && (drugToApprove["Verified"] = "true")
    ...
    if (product["Verified"] == "true"){
      ...
      pendingElements.push(
        <div className="card w-72 bg-base-100 shadow-xl">
          <figure className="max-h-[8em]"><img src={product["Image URL"]} alt={product["Medicine Name"]} /></figure>
          <div className="card-body max-h-[50em]">
            <h2 className="card-title flex flex-col items-start">
              <div>{product["Medicine Name"]}</div>
              <div className="p-1 w-full bg-[#A4CBB4] text-center text-xs font-medium rounded-full">{product["Manufacturer"]}</div>
            </h2>
            ...
            <div className="tooltip" data-tip={product["Composition"]}>
              ...
              <div className={`badge text-white ${product["Verified"] == 'true' ? 'badge-success' : 'badge-warning'}`} >
                {product["Verified"] == 'true' ? 'Approved' : 'Pending'}
              </div>
            </div>
              <button className="btn bg-success p-0 rounded-full font-mono font-bold text-sm text-center" onClick={()=>handleApprove(product["Medicine Name"])}>Approve</button>
          </div>
        </div>
      )
    }
